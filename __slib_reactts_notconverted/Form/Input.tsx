...
import {alert_add} from "@/_slib_reactts/Alert";
import {sDialog} from "@/_slib_reactts/Dialog";
import sDateTime from "@/_slib_reactts/_lib/dateTime";
import {isDev} from "@/_slib_reactts/_globalFunctions";
import $ from 'jquery';

const Inputmask = (typeof (window) != "undefined") ? require("@/_slib_reactts/Form/plugins/jquery.mask.min") : {};
...
export const Input: React.FC<PropsInput> = ({
    ...
    const refMain = componentRef || useRef<any>();
    ...
    const Focus = useCallback((e: any) => {
        ...
        if (mask?.length > 0 && e.target && Inputmask) {
            isDev && console.log("MASK var", mask);
            // @ts-ignore
    ...
    const Blur = useCallback((e: any) => {
        ...
        if (dateGecmisKontrol && e.target.value !== "") {
            if (e.target.value < new sDateTime().today()) {
                sDialog({
