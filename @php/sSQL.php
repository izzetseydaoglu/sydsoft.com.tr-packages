 ...
 */
   ...
   class sSQL {
	  ...
	  private string|null $mysql_db = null;
	  private string|null $cacheID = null;
	  ...
	  public function getResult(string $tableName = null, array $where = [], array $otherConfig = null, bool $noCache = false) {
		 if (empty($tableName) || (!count($where)) > 0) return false;
		 $sPDO = new sPDO($this->config_sSQL);
		 $sPDO->setDb($this->mysql_db);
		 $sPDO->setTable($tableName);
		 $sPDO->setWhere($where);

		 if ($otherConfig) {
			if (isset($otherConfig["SELECT"])) $sPDO->setSelect($otherConfig["SELECT"]);
			if (isset($otherConfig["GROUP"])) $sPDO->setGroup($otherConfig["GROUP"]);
			if (isset($otherConfig["ORDER"])) $sPDO->setOrder($otherConfig["ORDER"]);
			if (isset($otherConfig["LIMIT"])) $sPDO->setLimit($otherConfig["LIMIT"]);
		 }
		 if ($noCache) {
			$result = $sPDO->fetch();
			if ($result) {
	  ...
	  public function getResults(string $tableName = null, array $where = [], array $otherConfig = null, bool $noCache = false) {
		 ...
		 $sPDO->setDb($this->mysql_db);
		 $sPDO->setTable($tableName);
		 if (count($where) > 0) $sPDO->setWhere($where);
		 if ($otherConfig) {
			if (isset($otherConfig["SELECT"])) $sPDO->setSelect($otherConfig["SELECT"]);
			if (isset($otherConfig["GROUP"])) $sPDO->setGroup($otherConfig["GROUP"]);
			if (isset($otherConfig["ORDER"])) $sPDO->setOrder($otherConfig["ORDER"]);
	  ...
	  public function insert(string $tableName = null, array $data = []): bool|int|string {
		 if (empty($tableName) || (!count($data)) > 0) return false;
		 $sPDO = new sPDO($this->config_sSQL);
		 $sPDO->setDb($this->mysql_db);
		 $sPDO->setTable($tableName);
	  ...
	  public function update(string $tableName = null, array $where = [], array $data = []): bool|array {
		 if (empty($tableName) || (!count($where)) > 0 || (!count($data)) > 0) return false;
		 $sPDO = new sPDO($this->config_sSQL);
		 $sPDO->setDb($this->mysql_db);
		 $sPDO->setTable($tableName);
		 $sPDO->setWhere($where);
