...
class SYDSQL
{
	...
	private ?SYDMEMCACHE $cache = null;
	...
	public function __construct(array $config_sSQL = [])
	{
		...
		} elseif (isset($GLOBALS['config_sSQL'])) {
	...
	private function getMemcache(): SYDMEMCACHE
	{
		if ($this->cache === null) {
			$this->cache = new SYDMEMCACHE();
		}
		return $this->cache;
	}
	...
	public function getResultsWithFilter(
		string $tableName = null,
		array $where = [],
		array $otherConfig = null,
		bool $noCache = false,
		...
		if ($result) {
			$count = $this->getResults(
				$tableName,
				$where,
				['SELECT' => 'COUNT(*) as count', 'LIMIT' => null],
				$noCache,
			);
			return [
				'rows' => $result,
	...
	public function insertUpdate(
		...
		if (empty($tableName) || count($data) === 0 || count($where) === 0) {
			return false;
		}
	...
	{
		return str_shuffle(
			substr(str_repeat(time() . md5(mt_rand()), 21 + (int) $length), 0, (int) $length),
		);
	}
