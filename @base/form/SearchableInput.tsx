...
const Component: React.ForwardRefRenderFunction<handle, Props> = (
  ...
  const [loading, setLoading] = useState<boolean>(isDataFromApi);
  ...
  useEffect(() => {
    if (parentInputValue) clear(false);
  }, [parentInputValue]);
        ...
        endAdornment={
          !disabled && (
            <div style={{ marginRight: 5 }} tabIndex={-1}>
              <Button title={"Temizle"} tabIndex={-1} hidden={!(text && text.length > 0)} onClick={() => clear(true, true)} onlyIcon={<Icon name={"close"} style={{ color: "#444" }} />} />
              {endAdornment}
              <Button
                tabIndex={-1}
